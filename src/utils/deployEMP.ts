...
export const deployEMP = async (values: EMPParameters, network: ethers.providers.Network, signer: ethers.Signer) => {
    ...
    const { disputeBondPct, sponsorDisputeRewardPct, disputerDisputeRewardPct, excessTokenBeneficiary } = values
    ...
    const params = {
        ...
        sponsorDisputeRewardPct: {
            rawValue: toWei(sponsorDisputeRewardPct ? `${sponsorDisputeRewardPct / 100}` : "0.05"), // 0.05 -> 5% reward for sponsors who are disputed invalidly.
        },
        disputerDisputeRewardPct: {
            rawValue: toWei(disputerDisputeRewardPct ? `${disputerDisputeRewardPct / 100}` : "0.2"), // 0.2 -> 20% reward for correct disputes.
        },
        minSponsorTokens: {
