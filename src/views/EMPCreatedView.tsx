...
export const EMPCreatedView: React.FC = () => {
  ...
  const [empState, setEMPState] = useState<EMPDataParsed | undefined>(
    undefined
  );
  const [collateralState, setCollateralState] = useState<
    CollateralInfo | undefined
  >(undefined);
  const [collateralInstance, setCollateralInstance] = useState<
    ethers.Contract | undefined
  >(undefined);
  const [
    showContractDeploymentDetails,
    setShowContractDeploymentDetails,
  ] = useState(false);
  ...
  const getCollateralInfo = async (collateralInstanceNew: ethers.Contract) => {
    const collateralDecimals = (
      await collateralInstanceNew.decimals()
    ).toString();
  ...
  useEffect(() => {
    if (empAddress && signer !== null && address) {
      const getAllEMPData = async () => {
        ...
        const empInterface = new ethers.utils.Interface(
          umaABIs.get("ExpiringMultiParty") as any
        );
        if (!empInterface) {
          throw new Error("Invalid ExpiringMultiParty Interface");
        }

        const empInstance: ethers.Contract = new ethers.Contract(
          empAddress,
          empInterface,
          signer
        );
        ...
        const collateralRequirementPercentage = parseFloat(
          formatUnits(newState.collateralRequirement, collateralDecimals)
        ).toString();
  ...
  if (
    ...
    return (
      ...
      >
        ...
        <Dialog
          maxWidth="md"
          open={showContractDeploymentDetails}
        ...
        >
          ...
          >
            ...
            <ContractDetailItem
              ...
              value={`${empState.tokenName}`}
            />
            <ContractDetailItem
              label="Synthetic symbol"
              value={`${empState.tokenSymbol}`}
            />
            <ContractDetailItem
              label="Expiration date"
              value={`${empState.expireDate}`}
            />
            <ContractDetailItem
              label="Collateral address"
              value={empState.collateralCurrency}
            />
            <ContractDetailItem
              label="Price identifier"
              value={`${empState.priceIdentifier}`}
            />
            <ContractDetailItem
              label="Collateral requirement"
              value={`${empState.collateralRequirement}`}
            />
            <ContractDetailItem
              label="Minimum sponsor tokens"
              value={`${empState.minSponsorTokens}`}
            />
            <ContractDetailItem
              label="Liquidation liveness"
              value={`${empState.liquidationLiveness}`}
            />
            <ContractDetailItem
              label="Withdrawal liveness"
              value={`${empState.withdrawalLiveness}`}
            />
            <ContractDetailItem
              label="Dispute bond percentage"
              value={`${empState.disputeBondPercentage}`}
            />
            <ContractDetailItem
              label="Sponsor dispute reward percentage"
              value={`${empState.sponsorDisputeRewardPercentage}`}
            />
            <ContractDetailItem
              label="Disputer dispute reward percentage"
              value={`${empState.disputerDisputeRewardPercentage}`}
            />
            <ContractDetailItem
              label="Financial product library address"
              value={`${FINANCIAL_PRODUCT_LIBRARY}`}
            />
          </DialogContent>
        </Dialog>
...
const ContractDetailItem: React.FC<ContractDetailItemProp> = ({
  label,
  value,
}) => {
  return (
    <p style={{ fontSize: "0.9em" }}>
      {label}: <span style={{ fontSize: "0.8em" }}>{value}</span>
    </p>
  );
};
